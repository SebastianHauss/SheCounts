...
function buildTocFromHeadings(articleBodyEl, articleId) {
  ...
  if (headings.length === 0)
    headings = Array.from(articleBodyEl.querySelectorAll('h2'));
  ...
  const TOC_RULES_BY_ARTICLE = {
    ...
    '2b0be6ac-3584-4a01-aec0-5ee9df41e0b7': [
      {
        key: 'budgetierung',
        label: 'Budgetierung',
        match: /\bbudgetierung\b/i,
      },
      {
        key: '503020',
        label: '50-30-20 Regel',
        match: /50\s*[-–—]?\s*30\s*[-–—]?\s*20\s*[-–—]?\s*regel/i,
      },
      { key: 'tools', label: 'Tools und Tipps', match: /\btools\b.*\btipp/i },
      {
        key: 'vorteile',
        label: 'Vorteile der Budgetierung',
        match: /\bvorteile\b.*\bbudget/i,
      },
      {
        key: 'fehler',
        label: 'Fehler vermeiden',
        match: /\bh[aä]ufige\b.*\bfehler\b|\bfehler\b.*\bvermeid/i,
      },
    ...
    '89d9c07d-b318-4658-b3b4-b5f0ba7a60f6': [
      {
        key: 'warum',
        label: 'Warum Sparen wichtig ist?',
        match: /warum\s+sparen\s+wichtig\s+ist/i,
      },
      {
        key: 'psychologie',
        label: 'Psychologie des Sparens',
        match: /psychologie\s+des\s+sparens/i,
      },
      {
        key: 'wieviel',
        label: 'Wie viel sparen?',
        match: /wie\s+viel\s+sparen/i,
      },
      {
        key: 'strategie',
        label: 'Richtige Strategie finden',
        match: /strategie\s+finden/i,
      },
      {
        key: 'ziele',
        label: 'Sparziele definieren',
        match: /sparziele\s+definieren/i,
      },
      {
        key: 'fehler',
        label: 'Fehler vermeiden',
        match: /fehler\s+beim\s+sparen|fehler\s+vermeid/i,
      },
      { key: 'fazit', label: 'Fazit', match: /^\s*fazit\s*$/i },
    ],
    ...
    '732e4a48-48ce-4dd5-b24d-27be976d555e': [
      {
        key: 'beziehungen',
        label: 'Geld & Beziehungen',
        match: /geld\s+in\s+beziehungen/i,
      },
      {
        key: 'kommunikation',
        label: 'Kommunika&shy;tion als Grundlage',
        match: /offene\s+kommunikation\s+als\s+grundlage/i,
      },
      {
        key: 'konten',
        label: 'Konten',
        match: /gemeinsame\s+oder\s+getrennte\s+konten/i,
      },
      {
        key: 'fairness',
        label: 'Finanzielle Fairness',
        match: /finanzielle\s+fairness/i,
      },
      { key: 'ziele', label: 'Ziele definieren', match: /ziele\s+definieren/i },
      {
        key: 'konflikte',
        label: 'Konflikte lösen',
        match: /konflikte\s+l[oö]sen/i,
      },
      {
        key: 'absicherung',
        label: 'Absicherung für beide Seiten',
        match: /absicherung\s+f[uü]r\s+beide\s+seiten/i,
      },
      { key: 'fazit', label: 'Fazit', match: /^\s*fazit\s*$/i },
    ],
    ...
    'a9ae7d56-7047-4f8e-995c-d298644a37f5': [
      {
        key: 'abhaengigkeit',
        label: 'Finanzielle Abhängigkeit',
        match: /finanzielle\s+abh[aä]ngigkeit/i,
      },
      {
        key: 'unsichtbare',
        label: 'Unsichtbare Arbeit hat einen Preis',
        match: /unsichtbare\s+arbeit\s+hat\s+einen\s+preis/i,
      },
      {
        key: 'warnsignale',
        label: 'Warnsignale in der Beziehung',
        match: /warnsignale\s+in\s+der\s+beziehung/i,
      },
      {
        key: 'selbststaendigkeit',
        label: 'Finanzielle Selbststän&shy;digkeit',
        match: /finanzielle\s+selbstst[aä]ndigkeit/i,
      },
      {
        key: 'trennung',
        label: 'Trennung, Scheidung und Realität',
        match: /trennung\s*,\s*scheidung\s+und\s+realit[aä]t/i,
      },
      {
        key: 'altersarmut',
        label: 'Altersarmut von Frauen',
        match: /altersarmut\s+(von\s+)?frauen/i,
      },
      { key: 'fazit', label: 'Fazit', match: /^\s*fazit\s*$/i },
    ],
    ...
    '59b5462a-d2a4-413e-8491-f5027baf8517': [
      {
        key: 'minimalismus',
        label: 'Was ist finanzieller Minimalismus?',
        match: /was\s+(ist|bedeutet)\s+finanzieller\s+minimalismus/i,
      },
      {
        key: 'weniger',
        label: 'Warum weniger oft mehr ist?',
        match: /warum\s+weniger\s+oft\s+mehr\s+ist/i,
      },
      {
        key: 'konsumfallen',
        label: 'Konsumfallen erkennen',
        match: /konsumfallen\s+erkennen/i,
      },
      {
        key: 'bewusst',
        label: 'Geld bewusst einsetzen',
        match: /geld\s+bewusst\s+einsetzen/i,
      },
      {
        key: 'schritte',
        label: 'Schritte im Alltag',
        match: /schritte\s+im\s+alltag/i,
      },
      {
        key: 'langfristig',
        label: 'Langfristiger Effekt',
        match: /langfristiger\s+effekt/i,
      },
      { key: 'fazit', label: 'Fazit', match: /^\s*fazit\s*$/i },
    ],
  };
  ...
  const TOC_RULES =
    articleId && TOC_RULES_BY_ARTICLE[articleId]
  ...
  try {
    ...
    if (dateEl) {
      if (parsed.dateText) {
        ...
        dateEl.textContent = new Date(article.createdAt).toLocaleDateString(
          'de-DE'
        );
      } else {
        dateEl.textContent = '';
        ...
        if (container && topTwo.length > 0) {
          ...
          for (const rec of topTwo) {
            ...
            const recRes = await fetch(
              `http://localhost:8080/api/articles/${rec.id}`,
              {
                credentials: 'omit',
              }
            );
